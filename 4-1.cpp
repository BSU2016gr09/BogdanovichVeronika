...
void sum(int A[],int B[], int C[], int N)
{
 int i=0, tmp=0;
	 while (i<N)
	 {
       C[i]=A[i]+B[i]+tmp;
	   tmp=C[i]/10;
	   C[i]=C[i]%10;//Тестировать не пробовала???? Пошагово выполнять???? Не????? А так бы увидела, что в этом цикле ты зацикливаешься ибо нет нигде изменения значения i. Так все время и "крутится" i=0
	 }
}
void dif(int A[], int B[],int D[], int N)
{
